...
| ------------------------------ | -------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------- |
| **Claude Code (terminal CLI)** | macOS terminal, Opus 4.7               | Brainstorming, design doc, implementation plan, multi-file scaffolding, running the test/lint loop end-to-end, AI bug logging.   |
| **Cursor Composer (`Cmd+I`)**  | Cursor IDE, Claude Sonnet 4.6 / 4.7    | Whole-file generations against a paste-in spec — design tokens, theme provider, repository pattern, board page shell.            |
| **Cursor Chat (`Cmd+L`)**      | Cursor IDE, Claude Sonnet 4.6          | Single-file refactors, "convert this `forEach` to `arrayMove`", inline doc lookups, "why is this Tailwind class doing nothing?". |
...
The split mattered: I used Claude Code for anything that crossed file boundaries or needed the conversation history, and Cursor for tight in-file work where the diff was the contract.
...
That prompt produced commits `66902af` (deps), `9245817` (shadcn init), and `6c0d061` (theme + layout shell — explicitly attributed to Cursor composer).
...
Chat prompt while staring at a Playwright failure where the "Saved" indicator never appeared:
...
Single-file extract, two call-sites updated. Took ~3 minutes. The reason it's representative: this is exactly the kind of mechanical refactor Composer is best at — the spec is unambiguous, the edits are local, and "do not break the convention" is something Claude knows how to honor when you name the convention explicitly.
...
- **Conventional commits + progressive committing.** Telling the model to commit per logical unit produced a clean `git log --oneline` (39 commits) that doubles as a changelog.
...
Of 39 commits, 8 carried real bugs from AI-generated code. Of those:
...
6. **Pair tools to their strengths.** Claude Code for multi-file + conversation history. Cursor Composer for whole-file edits against a paste-in spec. Cursor Chat for single-file refactors. Don't blur the lines — using Chat for a multi-file refactor produces incomplete diffs; using Composer for a one-line tweak is overkill.
