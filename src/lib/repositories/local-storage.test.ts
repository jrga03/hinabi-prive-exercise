...
describe('LocalStorageProjectRepository', () => {
  ...
  it('update rejects with NotFoundError for missing id', async () => {
    const repo = new LocalStorageProjectRepository()
    await expect(repo.update('00000000-0000-4000-8000-000000000000', { title: 'x' })).rejects.toBeInstanceOf(
      NotFoundError,
    )
  })
...
describe('LocalStorageTaskRepository', () => {
  ...
  it('delete cascades to all descendants (multi-level)', async () => {
    ...
    const child = await repo.create(makeTaskInput(pid, { title: 'child', parentTaskId: parent.id }))
    const grand = await repo.create(
      makeTaskInput(pid, { title: 'grand', parentTaskId: child.id }),
    )
